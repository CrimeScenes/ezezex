...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        local timeToPredict = getgenv().Triggerbot.Prediction.Ground
...
local function isIgnoringKnife()
    ...
    if currentTool then
        ...
        -- Updated knife check to quickly react when switching between knife and gun
        if toolName == "knife" or toolName == "katana" or toolName == "[knife]" or toolName == "[katana]" then
            return true
        end
    end
    return false
            ...
            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                ...
                if closestPart then
                    if os.clock() - lastClickTime >= getgenv().Triggerbot.Cooldown then
                        ...
                        -- Ensure to shoot only when targeting the predicted position or actual target
                        adjustAimForTarget(closestPart.Position)

                        -- Updated knife check to prevent shooting with a knife
